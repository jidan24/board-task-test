...
import { IonButton, IonIcon, IonPopover, IonList, IonItem, IonInput, useIonAlert, useIonToast } from '@ionic/react';
  ...
  return (
    <Draggable draggableId={column.id} index={index}>
      {(provided) => (
        ...
        >
          ...
          <div 
            ...
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px 16px 0', marginBottom: '8px' }}
          >
          ...
          {isEditing ? (
            <IonInput 
              ...
              onIonBlur={submitEdit}
              onKeyDown={e => e.key === 'Enter' && submitEdit()}
              autoFocus
              style={{ fontWeight: 'bold', color: 'var(--ion-text-color)', width: 'auto' }}
            />
          ) : (
          ...
          {/* Options Button */}
          <div 
            onClick={(e) => { e.persist(); setPopoverEvent(e.nativeEvent); setShowPopover(true); }}
            style={{ 
              display: 'flex',
              justifyContent: 'center',
              alignItems: 'center',
      ...
      <IonPopover
        ...
        >
          <IonList lines="none" style={{ padding: '4px 0' }}>
            <IonItem button detail={false} onClick={() => { setIsEditing(true); setShowPopover(false); }} style={{ '--min-height': '40px' }}>
      ...
      <div style={{ padding: '8px 16px' }}>
        <IonButton fill="clear" expand="block" color="medium" onClick={handleAddTask} style={{ textTransform: 'none', justifyContent: 'flex-start' }}>
